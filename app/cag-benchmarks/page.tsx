...
const CAG_CASES: AuditCase[] = [
  ...
  {
    id: "dfccil-wdfc",
    name: "Western Dedicated Freight Corridor (WDFC) — Phase 1",
    agency: "DFCCIL / Ministry of Railways",
    state: "Maharashtra & Gujarat",
    costCr: 18500,
    reportRef: "CAG Performance Audit on Dedicated Freight Corridor (Report No. 7 of 2020)",
    actualDelayMonths: 36,
    earlyWarningMonths: 16,
    keyDrivers: [
      "Arbitration on circle rate compensation under Section 20F of Railways Act",
      "Forest clearance tree enumeration delays across Dahanu & Palghar sections",
      "LAO document rejection rate spike due to joint ownership claims",
    ],
    findings:
      "Piecemeal land acquisition caused prolonged corridor gaps, preventing continuous automated track laying machines (NTC) from deployment.",
    leadTimeGainText:
      "Predictive Regressor flagged a 30-month delay trajectory in Year 1 based on arbitration pileup and forest clearance lag.",
  },
];
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl space-y-6 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Implementing Agency: <b className="text-slate-200">{selectedCase.agency}</b> · State: <b className="text-slate-200">{selectedCase.state}</b></p>
            </div>
            <div className="sm:text-right">
              <span className="text-[10px] text-slate-400 font-mono block">Verified Cost</span>
